...
/*nv_flalg_enable,shield port switch when nv77=4 detected*/
...
/*unqiue serial number control*/
static char read_unique_sn(void)
{
	return zte_usb_parameters.unique_sn;
}
static void write_unique_sn(char enable)
{
	zte_usb_parameters.unique_sn= !!enable;
}

/*unqiue serial number string*/
static char* read_unique_sn_str(void)
{
	return zte_usb_parameters.unique_sn_str;
}
static void write_unique_sn_str(char* param)
{
	strncpy(zte_usb_parameters.unique_sn_str,		
			param, 
			sizeof(zte_usb_parameters.unique_sn_str));                               
	zte_usb_parameters.unique_sn_str[sizeof(zte_usb_parameters.unique_sn_str) - 1] = 0;  
	return ; 
}
...
static void iSerialNumber_filter(struct usb_composite_dev *cdev)
{
	...
	else if (read_unique_sn() && !usb_cdrom_is_enable()){
		strings_dev[STRING_SERIAL_IDX].s = read_unique_sn_str();
		pr_info("usb: %s unique serial number is %s\n", __func__, strings_dev[STRING_SERIAL_IDX].s);
		}
	if (cdev)			
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		iSerialNumber_filter(cdev); //xingbeilei_20120512	
...
//for unique serial number,wangzy
static int get_mac_address(unsigned char *addr)
{	int rc = 0;	
	...
	if(!rc) {		
		...
		pr_info("usb: %s MAC: %02X:%02X:%02X:%02X:%02X:%02X\n", __func__,
...
static ssize_t store_unique_sn_str(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t size)
{
	int value;
	...
	char unique_sn[64] = {0};
	sscanf(buf, "%d", &value);
	pr_info("usb: %s, %d: %d unique sn is %s\n",
	       __FUNCTION__, __LINE__, value, value?"enabled":"disabled");
	write_unique_sn(value);
	if (value) {
		get_mac_address(addr);
		snprintf(unique_sn, sizeof(unique_sn), "%s%02X%02X%02X",
			 serial_string, addr[3], addr[4], addr[5]);
		write_unique_sn_str(unique_sn);
	}
	return size;
}

static ssize_t show_unique_sn_str(struct device *dev,				  
							struct device_attribute *attr,				  
							char *buf)
{	
	int i = 0;	
...
static DEVICE_ATTR(unique_sn, 0664, show_unique_sn_str, store_unique_sn_str);
