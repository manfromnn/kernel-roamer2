...
#include "MCH_ZV790(M)_R01_V03_bin.c"

//#include "MCH_DAW550(E)_R01_V03_bin.c"
...
//---------------------------------
static struct melfas_ts_data *melfas_download_ts;
...
int mcsdl_download_binary_data(struct melfas_ts_data *ts)
{
	int ret;
	unsigned short addr=ts->client->addr;
	melfas_download_ts=ts;
	...
	melfas_download_ts->client->addr=addr;
	return ( ret == MCSDL_RET_SUCCESS );
}



int mcsdl_download_binary_file(struct melfas_ts_data *ts)
{
	int ret;
...
static void download_gpio_init(void)
{
	//gpio_free(82);
	
	int rc;
	printk("download_gpio_init\n");
...
static void download_gpio_backup(void)
{
	
	printk("download_gpio_backup\n");
	gpio_tlmm_config(GPIO_CFG(131, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(132, 2, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
}
...
static int mcsdl_i2c_prepare_program(void)
{
	...
	//------------------------------------------------------
	bRet = _i2c_read_( MCSDL_I2C_SLAVE_ADDR_ORG, &i2c_buffer[3], 1 );

	if( (bRet == 0) || (i2c_buffer[3] != MCSDL_I2C_ACK_PREPARE_PROGRAM))
		goto MCSDL_I2C_PREPARE_PROGRAM_FINISH;
...
BOOLEAN _i2c_read_( UINT8 slave_addr, UINT8 *pData, UINT8 cLength)
{
	BOOLEAN bRet;
		...
		melfas_download_ts->client->addr=(unsigned short)slave_addr;
		//bRet = baseband_i2c_read( slave_addr, pData, cLength );
		//bRet = i2c_smbus_read_i2c_block_data( melfas_download_ts->client,0, cLength, pData );
		bRet = i2c_master_recv(melfas_download_ts->client, pData, cLength);
		// or
...
BOOLEAN _i2c_write_(UINT8 slave_addr, UINT8 *pData, UINT8 cLength)
{
	BOOLEAN bRet;
		...
		melfas_download_ts->client->addr=(unsigned short)slave_addr;
		//bRet = i2c_smbus_read_i2c_block_data( melfas_download_ts->client,0, cLength, pData );
		//bRet = i2c_smbus_write_i2c_block_data(melfas_download_ts->client,0,cLength,pData);
		bRet = i2c_master_send(melfas_download_ts->client, pData, cLength);
		//	or
