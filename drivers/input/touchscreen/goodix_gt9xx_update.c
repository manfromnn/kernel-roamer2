...
extern void gtp_irq_enable(struct goodix_platform_data *ts);
extern void gtp_irq_disable(struct goodix_platform_data *ts);
...
#if GTP_HEADER_FW_UPDATE
/*
EC617002132375,【自由测试】【设置】手机加密不成功
goodix 9xx驱动的问题

加密过程需要卸载data分区。如果有其他应用打开data分区下的文件，则会出现卸载文件系统不成功的问题。
在触摸屏驱动里，gup_check_fs_mounted检测data分区是否已经挂载。这个函数里调用kern_path来判断，在kern_path的complete_walk函数里会增加/data/目录项，但没调用path_put释放目录项。这样就导致无法卸载data分区，从而出现无法加密的问题。

我看kernel里的代码都这么使用kern_path，在调用kern_path之后，有path_put的调用。
...
#if GTP_AUTO_UPDATE
u8 gup_init_update_proc(struct goodix_platform_data *ts)
{
