...
#if defined(CONFIG_TOUCHSCREEN_CYPRESS_UPDATE)
static int update_result_flag_cyttsp=0;
#endif
...
#define CY_WDG_TIMEOUT              msecs_to_jiffies(1000)
...
#if defined(CONFIG_TOUCHSCREEN_CY8CTST241_RESET)

static void cyttsp_timer_work_func(struct work_struct *work)
{
    struct cyttsp *ts = container_of(work, struct cyttsp, work);
    u8 heartBeatCounter;
    s32 retval;
    static u8 heartBeatCounterPrev = 0;


	//printk("xiayc00000: %s\n",__func__);
    /* Reset  timer */
	#if defined(CONFIG_MACH_MOONCAKE2)
	retval = ttsp_read_block_data(ts, CY_REG_BASE + 0x18, 1, &heartBeatCounter);	
	#else
	retval = ttsp_read_block_data(ts, CY_REG_BASE + 0x1C, 1, &heartBeatCounter);
	#endif
	//printk("xiayc: 20120104  %s. retval=%d\n",__func__,retval);
    if ( retval >= 0 )
    {
       /* tt_undef[8] is used as the Touch Screen Heart beat counter */
       if( heartBeatCounterPrev != heartBeatCounter)
   	   {
          heartBeatCounterPrev = heartBeatCounter;
		  mod_timer(&ts->timer, jiffies + CY_WDG_TIMEOUT);
          return;
       }
    }	
    /* We are not supposed to be here in mormal state 
    if yes, then touch screen is stuck and we need to re-apply power cycle*/
    //printk("xiayc: reset~~~~?\n");
	
	disable_irq_nosync(ts->irq);
	ts->platform_data->init(1);
	...
	//printk("xiayc:heartBeatCounterPrev=%d,heartBeatCounter=%d\n ",
	//	heartBeatCounterPrev, heartBeatCounter);
	heartBeatCounterPrev=0;
...
static int cyttsp_set_operational_mode(struct cyttsp *ts)
{
	int retval;
	int tries;
	#ifdef CONFIG_MACH_DEEPBLUE
	u8 cmd = CY_OPERATE_MODE+4;
	#else
	...
	do {
		...
		((xy_data.hst_mode & 0x70) == 0x00) &&
		(*((u8*)(&xy_data.hst_mode + 0x18)) != 0x00))&&
		//(tries++ < CY_DELAY_MAX));
		(tries++ < 5));
#else
	do {
	...
	DBG2(printk(KERN_INFO "%s: read tries=%d\n", __func__, tries);)
	printk(KERN_INFO "%s: read tries=%d\n", __func__, tries);
...
#define virtualkeys virtualkeys.cyttsp-i2c

#if defined (CONFIG_MACH_DEEPBLUE) || defined (CONFIG_MACH_ROAMER2)
static const char ts_keys_size[] = "0x01:139:45:520:70:50:0x01:102:120:520:70:50:0x01:158:200:520:70:50:0x01:217:265:520:70:50";
#else
			  ...
			  void *data)
{
	...
	len += sprintf(page + len, "%-20s %s\n", "name:","Cypress");
	//len += sprintf(page + len, "i2c address  : 0x%x\n", 0x22);
	//len += sprintf(page + len, "IC type    : %s\n", "3000 series");
	if(ts)
		len += sprintf(page + len, "%-20s 0x%02x%02x%02x_0x%02x%02x\n", "firmware version:", 
		ts->sysinfo_data.cid[0], ts->sysinfo_data.cid[1],ts->sysinfo_data.cid[2],
		ts->sysinfo_data.app_verh, ts->sysinfo_data.app_verl);	
	else
		len += sprintf(page + len, "%-20s %s\n", "firmware version:","unknown");	
	len += sprintf(page + len, "%-20s %s\n", "module:","CY8CTST241");
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_UPDATE
	len += sprintf(page + len, "%-20s 0x%x\n","update flag:", update_result_flag_cyttsp);
#endif
	if (off + count >= len)
		*eof = 1;
	if (len < off)
		   ...
		   unsigned long count, void *data)
{
	...
	const char *fw_name ="ZTE_OFILM_P752DV0602.hex";
	#else
	const char *fw_name ="ZTE_U805_TST242_V0615.hex";
	#endif
	update_result_flag_cyttsp=0;

	if(ts)
		cyttsp_force_update_fw_store(ts->pdev,NULL,fw_name,strlen(fw_name)+1);
	
	update_result_flag_cyttsp=2;
#endif
...
static int cyttsp_power_on(struct cyttsp *ts)
{
	...
	ts->sysinfo_data.app_verh = ts ->bl_data.appver_hi;
	ts->sysinfo_data.app_verl = ts ->bl_data.appver_lo;
	printk(KERN_INFO"%s:SI2: app_ver=%02X%02X  ",__func__,
		ts->sysinfo_data.app_verh, ts->sysinfo_data.app_verl);

	ts->sysinfo_data.cid[0] = ts->bl_data.cid_0;
	ts->sysinfo_data.cid[1] = ts->bl_data.cid_1;
	ts->sysinfo_data.cid[2] = ts->bl_data.cid_2;
	pr_info("%s: c_id=%02X%02X%02X ",__func__,
		ts->sysinfo_data.cid[0], ts->sysinfo_data.cid[1],ts->sysinfo_data.cid[2]);
...
static int cyttsp_suspend(struct cyttsp *ts)
{
	...
	if (ts->platform_data->use_sleep &&
		...
		#ifdef CONFIG_MACH_DEEPBLUE
		sleep_mode = CY_DEEP_SLEEP_MODE+4;
		#else
				...
				struct device_attribute *attr, char *buf)
{
	...
	const char *path="fw.bin";
	int retval = 0;

	retval = request_firmware(&cyttsp_fw, path, dev);
	if (retval < 0) {
		pr_err("%s: request %s failed(%d)\n", __func__,
						path, retval);
	} else {
		printk("%s request %s successed:%d bytes\n",__func__,
						path, cyttsp_fw->size);
				...
				int data_len, u8 *buf, bool force)
{
	...
	pr_info("Current firmware: 0x%02x%02x_0x%02x%02x\n", ts->sysinfo_data.app_idh, ts->sysinfo_data.app_idl,
		ts->sysinfo_data.app_verh, ts->sysinfo_data.app_verl);
	pr_info("New firmware: 0x%02x%02x_0x%02x%02x\n", appid_hi,appid_lo, appver_hi, appver_lo);

	if (force)
		fw_upgrade = 1;
	else
		if ((appid_hi == ts->bl_data.appid_hi) &&
			(appid_lo == ts->bl_data.appid_lo)) {
			if (appver_hi > ts->bl_data.appver_hi) {
				fw_upgrade = 1;
			} else if ((appver_hi == ts->bl_data.appver_hi) &&
					 (appver_lo > ts->bl_data.appver_lo)) {
					fw_upgrade = 1;
				} else {
					fw_upgrade = 0;
...
static void cyttspfw_upgrade(struct device *dev, bool force, const char *buf, size_t size)
{
	struct cyttsp *ts = dev_get_drvdata(dev);
	const struct firmware *cyttsp_fw;
	int retval = 0;
	...
	if (size > sizeof(fw_name) - 1)
	{
		pr_err("%s fw_name(%s) is too long\n",__func__,buf);
		return;
	}
	...
	retval = request_firmware(&cyttsp_fw, fw_name, dev);
	if (retval < 0) {
				...
				const char *buf, size_t size)
{
	...
	if (!ts->fw_loader_mode) {
		ts->fw_loader_mode = 1;
		cyttspfw_upgrade(dev, true, buf, size);
		ts->fw_loader_mode = 0;
	}
...
static DEVICE_ATTR(cyttsp_force_update_fw, 0644, cyttsp_update_fw_show,
				...
				const char *buf, size_t size)
{
	...
	if (!ts->fw_loader_mode) {
		ts->fw_loader_mode = 1;
		cyttspfw_upgrade(dev, false, buf, size);
		ts->fw_loader_mode = 0;
	}
	mutex_unlock(&ts->mutex);
...
static DEVICE_ATTR(cyttsp_update_fw, 0644, cyttsp_update_fw_show,
...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *pdev)
{
	...
	/* setup watchdog */
#if defined(CONFIG_TOUCHSCREEN_CY8CTST241_RESET)
	INIT_WORK(&ts->work, cyttsp_timer_work_func);
#else
	...
	if (ts->platform_data->use_mt) {
		...
		input_set_abs_params(input_device, ABS_MT_PRESSURE, 0, 0xFF, 0, 0);
		
		if (ts->platform_data->use_trk_id)
			input_set_abs_params(input_device, ABS_MT_TRACKING_ID,0, CY_NUM_TRK_ID, 0, 0);
	}
...
/*ergate*/
...
#else
	ts->fw_loader_mode = 0;
	retval = device_create_file(ts->pdev, &dev_attr_cyttsp_update_fw);
	if (retval) {
		printk(KERN_ERR "sysfs entry for firmware update failed\n");
		goto device_create_error;
	}

	retval = device_create_file(ts->pdev,
				&dev_attr_cyttsp_force_update_fw);
	if (retval) {
		printk(KERN_ERR "sysfs entry for force firmware update failed\n");
		goto device_create_error;
	}
	...
	if (retval) {
		printk(KERN_ERR "sysfs entry for file name selection failed\n");
		goto device_create_error;
	}
#endif
