...
static void focaltech_ts_work_func(struct work_struct *work)
{
	...
	else
	{
		...
		for (i = 0; i< ts->touch_event; i++)
		{
			...
			input_mt_sync(ts->input_dev);
			//printk("%s: xym finger=%d, xyz=%d %d %d, event_flag=%d, touch_id=%d\n", __func__, i, 
...
//		ts->gpio_irq = pdata->gpio_irq;
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, xres-1, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, yres-1, 0, 0);
