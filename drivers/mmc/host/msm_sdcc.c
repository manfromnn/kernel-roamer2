				...
				struct mmc_command *cmd, u32 *c)
{
	...
	*c |= (cmd->opcode | MCI_CPSM_ENABLE);
	...
	if ((((cmd->flags & MMC_RSP_R1B) == MMC_RSP_R1B))&&(cmd->opcode != MMC_SWITCH)){
...
msmsdcc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	mod_timer(&host->req_tout_timer,
			...
			(jiffies + msecs_to_jiffies(host->curr.req_tout_ms)));
...
static const struct mmc_host_ops msmsdcc_sd_ops = {
	.enable		= msmsdcc_enable,
	.disable	= msmsdcc_disable,
	.request	= msmsdcc_request,
	.set_ios	= msmsdcc_set_ios,
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	*/
	...
	host->early_suspend.level   = (EARLY_SUSPEND_LEVEL_STOP_DRAWING-10)+host->pdev_id;    //91 sd ,92 wifi , 93 emmc   jinzhiqiang change follow lianghouxin
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO ) ////|| host->pdev_id != 2)//shaohua add for wifi suspend
		   if (unlikely(work_busy(&mmc->detect.work)))
			rc = -EAGAIN;
                 else
			rc = mmc_suspend_host(mmc);
		else  if(mmc->card && host->clks_on){				
			clk_disable(host->clk);				
			clk_disable(host->pclk);				
			host->clks_on = 0;			
		}
