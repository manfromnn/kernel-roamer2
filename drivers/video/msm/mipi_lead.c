...
static int mipi_get_manufacture_icid(struct msm_fb_data_type *mfd)
{
	...
	int i ;
	 ...
	 struct mipi_manufacture_ic mipi_manufacture_icid[5] = {
	...
	{	lcd_panle_reset();	
		...
		if (mipi_manufacture_icid[i].readid_tx != NULL)
		{
...
static int mipi_lcd_on(struct platform_device *pdev)
{
	...
	switch(LcdPanleID){
		...
		case (u32)LCD_PANEL_4P0_OTM8018_CMI:
				mipi_dsi_cmds_tx(mfd, &lead_tx_buf, cmi_otm8018_display_on_cmds,ARRAY_SIZE(cmi_otm8018_display_on_cmds));
