...
#define TAOS_TRITON_TEST_OFFSET		0x1E
...
static u8 offset_status=0;
static u8 prox_offset_param = 0x7f;
static u8 prox_offset=0x7f; //equal to prox_offset_param
...
static u16 gain_trim_param = 100;
#elif defined(CONFIG_MACH_ATLAS32)
static u16 gain_trim_param = 240;
...
static u8 prox_pulse_cnt_param = 32;//16;
static u8 prox_gain_param = 0x60;//0x20;
...
static int enable_light_and_proximity(int mask)
{
	...
	if(mask==0x01)
	{
		...
		if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG|0x1E), prox_offset))) < 0) {
	...
	switch (cmd) {
		...
		case TAOS_IOCTL_CONFIG_GET:
			...
			if(!taos_cfgp->prox_pulse_cnt)
				taos_cfgp->prox_pulse_cnt=prox_pulse_cnt_param;
			taos_cfgp->prox_threshold_lo=taos_cfgp->prox_threshold_hi-50;
			offset_status=(taos_cfgp->prox_threshold_hi&0x07);
			prox_offset=(0x7f-(0x1f*offset_status));
...
printk(KERN_ERR "TAOS: prox_calibrate %d\n" , prox_cal_info[i].prox_data);								
				...
				taos_cfgp->prox_threshold_hi = prox_mean+80;
				taos_cfgp->prox_threshold_lo = prox_mean+50;
			 /*
				if((taos_datap->chip_type==TMD2771)|(taos_datap->chip_type==TMD27723)){				
				...
				*/ 
				...
				//if(taos_cfgp->prox_threshold_hi<=(sat_prox*1/5))
				if(prox_mean<=100)
				{
					if(prox_pulse==32){
					prox_offset_param-=0x1f;
					}
					if(prox_offset_param<=0x03)
					{
						prox_offset_param=0x03;
						prox_pulse+=1;	
					//	if((taos_datap->chip_type==TMD2771)||(taos_datap->chip_type==TMD27723)){
						//	count=20;
							//prox_pulse--;	
				...
				printk(KERN_ERR "taos:prox_pulse_cnt_param=%d s(%d) hi(%d) c(%d)\n",prox_pulse,sat_prox,taos_cfgp->prox_threshold_hi,count);
				if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG|0x1E), prox_offset_param))) < 0) {
                          	printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed\n");
                          	
				}	
				msleep(50);	
			}
			//while(((taos_cfgp->prox_threshold_hi>=sat_prox)||(taos_cfgp->prox_threshold_hi<(sat_prox*1/5)))&&(count<10));{
			while(((prox_mean>=800)||(prox_mean<=100))&&(count<30));{
				taos_cfgp->prox_pulse_cnt = prox_pulse;	
				taos_cfgp->prox_gain = prox_gain;	
				prox_offset=prox_offset_param;
				offset_status=((0x7f-prox_offset)/0x1f);	
			}
			//}
			//if (taos_cfgp->prox_threshold_lo < ((sat_prox*15)/100)) {				
				//taos_cfgp->prox_threshold_hi = ((sat_prox*20)/100);
			...
			if (taos_cfgp->prox_threshold_hi > 900) {				
				...
				taos_cfgp->prox_threshold_lo =860;
			}
			taos_cfgp->prox_threshold_hi=((taos_cfgp->prox_threshold_hi&0xFFF8)|offset_status);
...
static int taos_device_name(unsigned char *bufp, char **device_name) {
		...
		else if ((bufp[0x12]&0xf0)==0x00)
			taos_datap->chip_type=TSL2771;
		else 
			taos_datap->chip_type=TMD27723;
