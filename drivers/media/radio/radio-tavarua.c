	...
	*/
		if (adie_type_bahma) {
			...
			if (retval >= 0) {
				...
				msleep(TAVARUA_DELAY);//songyy add 0306
...
static int tavarua_fops_open(struct file *file)
{
	...
	if (!wait_for_completion_timeout(&radio->sync_req_done,
		msecs_to_jiffies(wait_timeout))) {
		retval = -1;
		printk(KERN_ERR "%s: Timeout waiting for initialization\n",
						__func__);
		FMDERR("Timeout waiting for initialization\n");
	}

	//Change to here@Andy
	radio->handle_irq = 0;//songyy add 0306


	/* get Chip ID */
	retval = tavarua_write_register(radio, XFRCTRL, CHIPID);
	if (retval < 0){
		printk(KERN_ERR "%s: get Chip ID, %d\n",
						__func__, retval);		
		goto open_err_all;
		}
	msleep(TAVARUA_DELAY);
	...
	if (radio->registers[XFRCTRL] != CHIPID){
		printk(KERN_ERR "%s: radio->registers[XFRCTRL] != CHIPID %d\n",
						__func__, radio->registers[XFRCTRL]);		
		goto open_err_all;
		}
    ...
    //Move to before previous line@Andy
	//radio->handle_irq = 0;//songyy add 0306
	radio->marimba->mod_id = SLAVE_ID_BAHAMA;
...
open_err_req_irq:
	if (radio->pdata->config_i2s_gpio != NULL){
		printk(KERN_WARNING DRIVER_NAME ": open_err_req_irq  FM_I2S_OFF \n");
		radio->pdata->config_i2s_gpio(FM_I2S_OFF);
		}
config_i2s_err:
	radio->pdata->fm_shutdown(radio->pdata);
...
static int  __init tavarua_probe(struct platform_device *pdev)
{
	...
	radio->lp_mode = 1;
	printk(KERN_ERR "%s: probe radio->handle_irq = 1\n",
					__func__);
	radio->handle_irq = 1;//@Andy
	/* init lock */
