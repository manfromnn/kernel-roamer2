...
static unsigned int ov2675_preview_VTS;
static unsigned int ov2675_capture_HTS;
static unsigned short ov2675_preview_maxlines;
static int m_60Hz = FALSE;

#define OV2675_GPIO_SWITCH_CTL     49

#define OV2675_GPIO_SWITCH_VAL     0
...
static int ov2675_video_config(void)
{
	...
	//ov2675_i2c_write(ov2675_client->addr, 0x339a, bns_1, 10);
	return rc;
}
...
//    printk("Preview_Maxlines=%d\n", ov2675_preview_maxlines);
    ...
    ret_h = ret_m = ret_l = 0;
    ov2675_i2c_read_byte(ov2675_client->addr,0x3028, &ret_h);
    ov2675_i2c_read_byte(ov2675_client->addr,0x3029, &ret_l);
	ov2675_i2c_read_byte(ov2675_client->addr,0x302c, &ret_m);
    ov2675_preview_HTS = ((ret_h << 8) | (ret_l & 0xff)) + (ret_m & 0xff);
    printk("ov2675_preview_HTS=%d\n", ov2675_preview_HTS);

	ret_h = ret_m = ret_l = 0;
    ov2675_i2c_read_byte(ov2675_client->addr,0x302a, &ret_h);
    ov2675_i2c_read_byte(ov2675_client->addr,0x302b, &ret_l);
	ov2675_i2c_read_byte(ov2675_client->addr,0x302d, &ret_m);
    ov2675_preview_VTS = ((ret_h << 8) | (ret_l & 0xff)) + (ret_m & 0xff);
    printk("ov2675_preview_VTS=%d\n", ov2675_preview_VTS);
...
static int ov2675_set_capture_exposure_gain(void)
{
   ...
   //calculate capture exp & gain
    ...
    ov2675_i2c_read_byte(ov2675_client->addr,0x3029, &ret_l);
	ov2675_i2c_read_byte(ov2675_client->addr,0x302c, &ret_m);
    ov2675_capture_HTS = (ret_h << 8) | (ret_l & 0xff);

    if(m_60Hz == TRUE)
    {
    Lines_10ms = 36000000 / ov2675_capture_HTS / 120;
    }
    else
    {
    Lines_10ms = 36000000 / ov2675_capture_HTS / 100;
    }
   ...
   iCapture_Gain = ov2675_gain * 75 / 100;
   //iCapture_Gain = ov2675_gain*3600/2400;//*2
   if(iCapture_Gain < 16)
   	{
   	iCapture_Gain = 16;
   	}
    //iCapture_Gain = ov2675_gain*(750)/(2000);
   ...
   // iCapture_Gain = ov2675_gain;
	...
	pr_err("ulCapture_Exposure_Gain=%d\n", ulCapture_Exposure_Gain);

    if(ulCapture_Exposure_Gain < Lines_10ms*16)
    ...
    else if(ulCapture_Exposure_Gain > Lines_10ms * 16 * 6)
    {
    ulCapture_Exposure = Lines_10ms * 6;
	iCapture_Gain = ulCapture_Exposure_Gain / ulCapture_Exposure;
    }
else
	{
	ulCapture_Exposure = (ulCapture_Exposure_Gain / 16 / Lines_10ms) * (Lines_10ms);
	iCapture_Gain = ulCapture_Exposure_Gain / ulCapture_Exposure;
	}
#if 0
...
static int ov2675_snapshot_config(void)
{
  ...
  ov2675_i2c_read_byte(ov2675_client->addr,0x3390, &bns_lv);
  ov2675_i2c_read_byte(ov2675_client->addr,0x301b, &avrg);
  ...
  {
   	bns_2+= 0x10; //enhance brightness level
	ov2675_i2c_write(ov2675_client->addr, 0x339a, bns_2, 10);
	}
...
static int ov2675_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	#if defined(CONFIG_MACH_MOONCAKE2) || defined(CONFIG_MACH_ATLAS40)  ||defined(CONFIG_MACH_SEANPLUS)
     rc = msm_camera_clk_switch(data, OV2675_GPIO_SWITCH_CTL, OV2675_GPIO_SWITCH_VAL);
    if (rc < 0)
    {
...
static int ov2675_sensor_probe(const struct msm_camera_sensor_info *info,struct msm_sensor_ctrl *s)
{
	...
	#if defined(CONFIG_MACH_MOONCAKE2) || defined(CONFIG_MACH_ATLAS40)  ||defined(CONFIG_MACH_SEANPLUS)
     rc = msm_camera_clk_switch(info, OV2675_GPIO_SWITCH_CTL, OV2675_GPIO_SWITCH_VAL);
    if (rc < 0)
    {
        pr_err("%s: msm_camera_clk_switch failed!\n", __func__);
        return rc;
...
{
	return msm_camera_drv_start(pdev, ov2675_sensor_probe,0);
}
