...
static void gpio_bt_config(void)
{
	if (machine_is_msm7627a_qrd1()){
		...
		printk("%s dm into_qrd1  gpio_bt_sys_rest_n=%d\n",__func__,gpio_bt_sys_rest_en);
	}
	else{
#if defined (CONFIG_MACH_ATLAS40) ||  defined (CONFIG_MACH_NICE) 
			gpio_bt_sys_rest_en = 115;
#elif defined (CONFIG_MACH_ROAMER2)
		  gpio_bt_sys_rest_en = 91;   //for P75D01 
#endif
		printk("%s dm into_else  gpio_bt_sys_rest_n=%d\n",__func__,gpio_bt_sys_rest_en);
	}
}
...
//compatible of qualcomm and broadcomm bluetooth chip     ZTE_BT_QXX_20101207 end
 ...
 struct platform_device msm_bcmsleep_device = {
	.name = "bcmsleep",
	.id		= -1,
